...
navigationBarTitleText: '该表存储申请信息',
},
}
</route>
<template>
...
const dataId = ref('')
const backRouteName = ref('LabApplicationList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab_application/labApplication/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab_application/labApplication/edit':'/lab_application/labApplication/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
