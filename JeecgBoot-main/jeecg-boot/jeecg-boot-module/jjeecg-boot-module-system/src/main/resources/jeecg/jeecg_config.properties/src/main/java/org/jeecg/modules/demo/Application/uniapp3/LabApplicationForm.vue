...
// 初始化数据
const initData = () => {
  http.get("/Application/labApplication/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/Application/labApplication/edit':'/Application/labApplication/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
