...
navigationBarTitleText: 'lab_group',
},
}
</route>
<template>
        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  ...
                  :label="get4Label('课题组编号')"
                  name='groupId'
                  prop='groupId'
                  placeholder="请选择课题组编号"
                  :rules="[
                  ]"
                  clearable
              />
...
const dataId = ref('')
const backRouteName = ref('LabGroupList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab/labGroup/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab/labGroup/edit':'/lab/labGroup/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
