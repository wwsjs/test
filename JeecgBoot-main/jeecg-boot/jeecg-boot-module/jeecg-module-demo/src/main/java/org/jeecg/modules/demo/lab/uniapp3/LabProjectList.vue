...
<template>
  <PageLayout navTitle="项目信息"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LabProjectData';
defineOptions({
  name: 'LabProjectList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lab/labProject/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lab/labProject/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
