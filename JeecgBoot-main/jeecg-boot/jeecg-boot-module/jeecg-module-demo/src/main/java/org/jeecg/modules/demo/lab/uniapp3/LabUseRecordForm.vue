        ...
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                ...
                v-model="myFormData['equipmentId']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('依托项目ID')"
                 labelWidth="100px"
                 type="sel_search"
                 name='projectId'
                 dict="lab_project,project_name,id"
                v-model="myFormData['projectId']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-date
                  :label="get4Label('开始日期')"
                  labelWidth="100px"
                  type="date"
                  name='startDate'
                  v-model:value="myFormData['startDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-date
                  :label="get4Label('结束日期')"
                  labelWidth="100px"
                  type="date"
                  name='endDate'
                  v-model:value="myFormData['endDate']"
              ></online-date>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-textarea
                :label="get4Label('用途或项目内容')"
                 labelWidth="100px"
                 type="textarea"
                 name='purpose'
                 prop='purpose'
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
               <wd-input
                   label-width="100px"
                   v-model="myFormData['actualHours']"
                   :label="get4Label('实际使用时间/小时')"
                   name='actualHours'
                   prop='actualHours'
                   placeholder="请选择实际使用时间/小时"
                   inputMode="numeric"
                   :rules="[
...
const dataId = ref('')
const backRouteName = ref('LabUseRecordList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab/labUseRecord/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab/labUseRecord/edit':'/lab/labUseRecord/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
