...
<template>
  <PageLayout navTitle="使用记录"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LabUseRecordData';
defineOptions({
  name: 'LabUseRecordList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lab/labUseRecord/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lab/labUseRecord/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
