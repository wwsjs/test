      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['projectNo']"
                  :label="get4Label('项目编号')"
                  name='projectNo'
                  prop='projectNo'
                  placeholder="请选择项目编号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('项目负责人ID')"
                 labelWidth="100px"
                 type="sel_search"
                 name='leaderId'
                 dict="sys_user,realname,id"
                v-model="myFormData['leaderId']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-multi
                :label="get4Label('项目成员ID')"
                 labelWidth="100px"
                 type="list_multi"
                 name='memberId'
                 dict="sys_user,realname,id"
                v-model="myFormData['memberId']"
              ></online-multi>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['contractAmount']"
                  :label="get4Label('合同经费')"
                  name='contractAmount'
                  prop='contractAmount'
                  placeholder="请选择合同经费"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('LabProjectList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab/labProject/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab/labProject/edit':'/lab/labProject/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
