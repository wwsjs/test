...
layout: 'default',
style: {
navigationBarTitleText: '人员信息（非系统用户）',
navigationStyle: 'custom',
},
}
</route>
<template>
  <PageLayout navTitle="人员信息（非系统用户）"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LabUserData';
defineOptions({
  name: 'LabUserList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lab/labUser/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lab/labUser/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
