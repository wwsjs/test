...
const dataId = ref('')
const backRouteName = ref('LabApplicationManageList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab/labApplicationManage/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab/labApplicationManage/edit':'/lab/labApplicationManage/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
