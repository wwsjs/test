      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['workNo']"
                  :label="get4Label('工号/学号')"
                  name='workNo'
                  prop='workNo'
                  placeholder="请选择工号/学号"
                  :rules="[
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['roomNo']"
                  :label="get4Label('办公室/实验室')"
                  name='roomNo'
                  prop='roomNo'
                  placeholder="请选择办公室/实验室"
                  :rules="[
                  ]"
                  clearable
              />
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <online-select
                :label="get4Label('人员类别')"
                 labelWidth="100px"
                 type="list"
                 name='personType'
                 dict="person_type"
                v-model="myFormData['personType']"
              ></online-select>
        </view>
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('管理员/组长/用户')"
                 labelWidth="100px"
                 type="list"
                 name='roleCode'
                 dict="lab_role_code"
                v-model="myFormData['roleCode']"
              ></online-select>
        </view>
          </wd-cell-group>
        </wd-form>
...
const dataId = ref('')
const backRouteName = ref('LabUserList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab/labUser/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab/labUser/edit':'/lab/labUser/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
