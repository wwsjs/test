...
<template>
  <PageLayout navTitle="申请管理"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LabApplicationManageData';
defineOptions({
  name: 'LabApplicationManageList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lab/labApplicationManage/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lab/labApplicationManage/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
