...
navigationBarTitleText: '实验室设备信息',
},
}
</route>
<template>
      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['equipmentCode']"
          ...
          <view class="{ 'mt-14px': 1 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['equipmentName']"
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <online-select
                :label="get4Label('设备状态')"
                 labelWidth="100px"
                 type="list"
                 name='status'
                 dict="equipment_status"
...
const dataId = ref('')
const backRouteName = ref('LabEquipmentList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/lab/labEquipment/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/lab/labEquipment/edit':'/lab/labEquipment/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
