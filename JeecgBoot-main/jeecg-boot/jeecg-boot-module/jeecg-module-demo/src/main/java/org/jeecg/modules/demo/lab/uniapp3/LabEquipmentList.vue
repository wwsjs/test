...
<template>
  <PageLayout navTitle="实验室设备信息"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LabEquipmentData';
defineOptions({
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lab/labEquipment/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lab/labEquipment/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
