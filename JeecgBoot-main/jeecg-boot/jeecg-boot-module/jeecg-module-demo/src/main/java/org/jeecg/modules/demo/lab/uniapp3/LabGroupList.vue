...
<template>
  <PageLayout navTitle="lab_group"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './LabGroupData';
defineOptions({
  name: 'LabGroupList',
  options: {
...
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/lab/labGroup/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/lab/labGroup/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
